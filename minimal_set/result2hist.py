...
if __name__ == "__main__":
    ...
    target_data_iou = target_data[:,6].astype(float)
    print(f"ious {target_data_iou.shape}")

    rng = [i/10 for i in range(0,10)]
    all_count = target_data_iou.shape[0]
    for i in rng:
        print(f"{i}:{target_data_iou[(target_data_iou > i) & (target_data_iou < i+0.1)].shape[0]/all_count}")
    ...
    # img_ids = [0]
    # windows = []
    # for id in [i for i in range(10,500,10)]:
